...
public class InputDirectExerActivity extends Activity {
    DBHelper mHelper;
    SQLiteDatabase db;
    Cursor cursor;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_inputdirectexer);
    ...
    {
        EditText dName = (EditText) findViewById(R.id.et_directename);
        EditText dTime = (EditText) findViewById(R.id.et_directetime);
        EditText dCal = (EditText) findViewById(R.id.et_directecal);

        String name = dName.getText().toString();
        int mtime = Integer.parseInt( dTime.getText().toString() );
        float total = Float.parseFloat( dCal.getText().toString() );

        try
        {
            // 문자열은 ''로 감싸야 한다.
            String query = String.format(
                    "INSERT INTO TODAY_EXER VALUES (null, datetime('now','+9 hours'), '%s', 0, %d, %f );", name, mtime, total );
